...
namespace LogAPI.Business.Services.Logging
{
    public class MongoLogRepository : ILogRepository
    {
        ...
        {
            var databaseName = configuration["LogSettings:DatabaseName"] ?? "DenemeApiLogs";
        ...
        public async Task<RequestResponseLog> GetRequestResponseLogByIdAsync(string id)
        {
            return await _requestResponseLogs.Find(log => log.Id == id).FirstOrDefaultAsync();
        }
        
        public async Task<ApiLog> GetApiLogByIdAsync(string id)
        {
            return await _apiLogs.Find(log => log.Id == id).FirstOrDefaultAsync();
        }
